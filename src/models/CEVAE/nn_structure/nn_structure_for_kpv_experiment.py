...
class KPVDistribution(AbstractDistribution):
    ...
    def __init__(self, n_hidden_dim: int = 20, n_learning_sample: int = 10):
        super(KPVDistribution, self).__init__(n_hidden_dim, n_learning_sample)
    ...
    def build_p_x_z(self):
        feature = nn.Sequential(nn.Linear(self.n_hidden_dim, 32),
                                nn.ReLU())
        self.pxz_mean_net = nn.Sequential(feature, nn.Linear(32, 4))
        self.pxz_sigma_net = nn.Sequential(feature, nn.Linear(32, 4), nn.Softplus())
    ...
    def build_q_z_xty(self):
        feature = nn.Sequential(nn.Linear(6, 64),
                                nn.ReLU(),
                                nn.Linear(64, 32),
                                nn.ReLU())
