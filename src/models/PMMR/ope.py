...
class PMMROPEModel:
    ...
    def __init__(self, base_model: PMMRModel, lam3_max: float, lam3_min: float,
    ...
    def predict(self, treatment: np.ndarray, covariate: np.ndarray):
        ...
        test_treatment_kernel = self.base_model.treatment_kernel_func.cal_kernel_mat(self.base_model.train_treatment,
                                                                                     treatment)
        pred = self.base_model.alpha.T @ (w_weight_pred * test_treatment_kernel)
        return pred.T
...
def pmmr_ope_experiments(data_config: Dict[str, Any], model_param: Dict[str, Any],
    ...
    if data_config["name"].startswith("demand"):
        ...
        raise ValueError
    base_model.fit(org_data, data_name)
    ope_model = PMMROPEModel(base_model=base_model, **model_param)
